...
class UTC_ReliableSim extends Sim {
	...
	public UTC_ReliableSim(double d) {
...
//		60=0.0166
		...
		Exp onlinePeriodNav = new Exp(0.1);
		Exp offlinePeriodNav = new Exp(0.0166);
		...
		OnOffRQN on0ff_bc2_app = new OnOffRQN("ON-OFF-BC2-APP", processingServeTimeBC, 1, onlinePeriodNav, offlinePeriodNav, duration);		
		QueueingNode mm1_bc2_tr = new QueueingNode("MM1-BC2-TR", transmissionServeTime, 1);
		...
		LftLsesBranch branchBC2 = new LftLsesBranch(new Node[] {sinkBC2, mm1_bc2_tr});
		...
		mm1_bc2_pr.setLink(new Link(on0ff_bc2_app));
		on0ff_bc2_app.setLink(branchBC2);
		mm1_bc2_tr.setLink(new Link(mm1_nav));
